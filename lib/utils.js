...
function getMergeData(prData) {
    ...
    const additionalAuthors = (((_c = (_b = prData.repository) === null || _b === void 0 ? void 0 : _b.pullRequest) === null || _c === void 0 ? void 0 : _c.commits.nodes) || [])
        .filter(n => !(n === null || n === void 0 ? void 0 : n.commit.message.startsWith('Merge branch'))) // filter out merge commits
        .flatMap(node => {
        var _a;
...
function getNextBranch(branch) {
    switch (branch) {
        ...
        default:
            throw Error(`Could not determine a forward merge branch for "${branch}". Supported branch inputs are support, master, main, and prerelease/minor`);
    }
}
