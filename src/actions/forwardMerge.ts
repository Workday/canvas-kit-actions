...
import {actionsCore as core} from '../lib'
...
function checkBranches(branch: string, nextBranch: string) {
  ...
  if (!nextBranch) {
    console.error(`The next branch name is not valid: ${nextBranch}`)
    process.exit(1)
  }
}
...
async function run() {
  ...
  const nextBranch = core.getInput('nextBranch')

  checkBranches(branch, nextBranch)
...
async function updateChangelog() {
  ...
  const contents = [
    ...header,
    ...sortedReleases
      .map((release: {title: string; contents: string[]}) => [release.title, ...release.contents])
      .flat(),
  ]
 ...
 * @param lines {string[]}
 */
function parseContents(lines: string[]) {
  const contents = []
